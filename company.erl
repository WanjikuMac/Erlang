...
setup_db() ->
    mnesia:create_table(employee, [{attributes, record_info(fields, employee)}, {disc_copies, [node(), nodes()]}]),
    mnesia:create_table(dept, [{attributes, record_info(fields, dept)}, {disc_copies, [node(), nodes()]}]),
    mnesia:create_table(project,[{attributes, record_info(fields, project)}, {disc_copies, [node(), nodes()]}]),
    mnesia:create_table(manager, [{attributes, record_info(fields, manager)},{disc_copies, [node(), nodes()]} ]),
    mnesia:create_table(at_dept, [{attributes, record_info(fields, at_dept)}, {disc_copies, [node(), nodes()]}]),
